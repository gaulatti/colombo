    ...
    S::Metadata: Metadata,
{
    ...
    // A new passive-mode command supersedes any accepted data socket that is
    // still waiting for a transfer command. Apache FtpServer closed that socket;
    // without this signal it remains attached for five minutes and can consume
    // the wrong command channel during appliance upload bursts.
    if let Some(stale_abort) = session.data_abort_tx.take() {
        let _ = stale_abort.try_send(());
    }
